import { TrendingUp, ChartColumn, Rocket, DollarSign, ArrowUpRight, CheckCircle2 } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto w-full relative z-10 px-4 py-6 lg:py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-stretch">
                    <motion.div
                        ...
                        className="bg-navy rounded-[2.5rem] p-6 lg:p-8 text-white relative overflow-hidden flex flex-col justify-between border border-white/10 shadow-3xl"
                    >
                        ...
                        <div>
                            <div className="w-14 h-14 rounded-2xl bg-primary/20 backdrop-blur-xl border border-white/10 flex items-center justify-center mb-6">
                                <TrendingUp className="w-7 h-7 text-primary" />
                            </div>

                            <h3 className="font-display text-2xl lg:text-3xl font-bold mb-4 leading-snug">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex items-center gap-4 group">
                                        ...
                                        <span className="font-body text-sm lg:text-base text-white/80 group-hover:text-white transition-colors">
                                            {item}
                                        </span>
                                    </div>
                                ))}
                        ...
                        <div className="mt-8 pt-6 border-t border-white/5 flex items-center gap-6">
                            <div>
                                <p className="font-body text-xs text-white/60 uppercase tracking-widest mb-1">Projected EBITDA Margin</p>
                                <p className="font-display text-2xl lg:text-3xl font-bold text-primary">22%</p>
                            </div>
                            <div className="w-px h-12 bg-white/10" />
                            <div>
                                <p className="font-body text-xs text-white/60 uppercase tracking-widest mb-1">Break-even Point</p>
                                <p className="font-display text-2xl lg:text-3xl font-bold text-primary">Year 2</p>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-8 items-stretch">
                        {metrics.map((metric, index) => (
                            <motion.div
                                ...
                                className="card-glass p-6 lg:p-7 rounded-[2rem] flex flex-col justify-between group hover:border-primary/30 transition-all duration-500 shadow-xl"
                            >
                                <div>
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="font-body text-[11px] uppercase tracking-[0.2em] text-primary font-bold">{metric.label}</span>
                                        <ArrowUpRight className="w-5 h-5 text-primary opacity-0 group-hover:opacity-100 transition-opacity" />
                                    </div>
                                    <p className="font-display text-2xl lg:text-3xl font-bold text-navy mb-3">{metric.value}</p>
                                    <p className="font-body text-sm lg:text-sm text-muted-foreground leading-relaxed">
                                        {metric.description}
                                    </p>
                                </div>
                            </motion.div>
